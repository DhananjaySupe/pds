    ...
    public function upload($params=array()){
        try {
            if(isset($params['base64'])){
                $this->result = $this->S3->putObject(array('Bucket'=>$this->S3_BUCKET,'Key'=>$params['key'],'Body'=>$params['base64'],'ContentEncoding'=>'base64','ContentType'=>$params['type'],'StorageClass'=>'STANDARD','ACL'=>'public-read'));
            } else {
                $this->result = $this->S3->putObject(array('Bucket'=>$this->S3_BUCKET,'Key'=>$params['key'],'SourceFile'=>$params['file'],'StorageClass'=>'STANDARD','ACL'=>'public-read'));
            }
            return true;
        } catch (S3Exception $e) {
