...
$(document).ready(function () {
    lostPeopleGrid = $('#lost-people-grid').DataTable({
        ...
        ajax: {
            type: "POST",
            url: SITE_URL + "/lost-people",
            beforeSend: function () { showLoader('.page-content', 'lostPeopleLoader'); },
			complete: function () { hideLoader('lostPeopleLoader'); },
            data: function (d) {
    ...
    $(document).on("click", "#lost-people-grid td a[data-action]", function (e) {
        e.preventDefault();
        var rowindex = $(this).closest('tr').index();
        var data = lostPeopleGrid.rows(rowindex).data().toArray();
        var action = $(this).data('action');
        switch (action) {
            case 'delete':
			lostPeopleAction('delete', data[0].id, data[0].text);
			break;
		}
	});
...
function refreshCommandbar() {
	var count = lostPeopleGrid.rows().count();
    var selected =  lostPeopleGrid.rows({ selected: true }).count() ;
...
function lostPeopleAction(action, id, text) {
    action = action || 'new'; id = id || 0; text = text || '';
    switch (true) {
        case action == "delete":
            bootbox.confirm({
                title: 'Delete lost people?',
                ...
                callback: function (result) {
                    if (result) {
                        ...
                        $.ajax({
                            type: "DELETE", url: SITE_URL + "/lost-people/delete?" + params, dataType: 'json',
