! function() {
    ...
    function n(e) {
        ...
        // Send AJAX request to update CodeIgniter language setting
        var xhr = new XMLHttpRequest();
        xhr.open("POST", SITE_URL + "/language/change", true);
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            console.log("Language changed successfully on server: " + response.language);
                        } else {
                            console.error("Error changing language on server: " + response.message);
                        }
                    } catch (e) {
                        console.error("Error parsing server response:", e);
                    }
                } else {
                    console.error("Error changing language on server. Status: " + xhr.status);
                }
            }
        };
        xhr.send("language=" + encodeURIComponent(e));
        document.getElementById("header-lang-img") && ("en" == e ? document.getElementById("header-lang-img").src = SITE_URL+"/assets/images/flags/us.svg" : "hi" == e ? document.getElementById("header-lang-img").src = SITE_URL+"/assets/images/flags/in.svg" : "mr" == e && (document.getElementById("header-lang-img").src = SITE_URL+"/assets/images/flags/in.svg"), localStorage.setItem("language", e), null == (a = localStorage.getItem("language")) && n(t), (e = new XMLHttpRequest).open("GET", SITE_URL+"/assets/lang/" + a + ".json"), e.onreadystatechange = function() {
            var a;
    ...
    function i() {
        var n, e = document.documentElement.getAttribute("data-layout"),
            ...
            t = JSON.parse(t);
        !t || "twocolumn" != e && "twocolumn" != t["data-layout"] || (document.querySelector(".navbar-menu") && (document.querySelector(".navbar-menu").innerHTML = d), (n = document.createElement("ul")).innerHTML = '<a href="#" class="logo"><img src="'+SITE_URL+'/assets/images/logo-sm.png" alt="" height="22"></a>', Array.from(document.getElementById("navbar-nav").querySelectorAll(".menu-link")).forEach(function(e) {
    ...
    function H() {
        Array.from(document.querySelectorAll("#notificationItemsTabContent .tab-pane")).forEach(function(e) {
            0 < e.querySelectorAll(".notification-item").length ? e.querySelector(".view-all") && (e.querySelector(".view-all").style.display = "block") : (e.querySelector(".view-all") && (e.querySelector(".view-all").style.display = "none"), e.querySelector(".empty-notification-elem") || (e.innerHTML += '<div class="empty-notification-elem">\t\t\t\t\t\t\t<div class="w-25 w-sm-50 pt-3 mx-auto">\t\t\t\t\t\t\t\t<img src="'+SITE_URL+'/assets/images/svg/bell.svg" class="img-fluid" alt="user-pic">\t\t\t\t\t\t\t</div>\t\t\t\t\t\t\t<div class="text-center pb-5 mt-2">\t\t\t\t\t\t\t\t<h6 class="fs-18 fw-semibold lh-base">Hey! You have no any notifications </h6>\t\t\t\t\t\t\t</div>\t\t\t\t\t\t</div>'))
        })
    }
